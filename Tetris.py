...
brd_color, bg_color, txt_color, title_color, info_color = white, black, white, colors[
    3], colors[0]
...
def drawBlock(block_x, block_y, color, pixelx=None, pixely=None):
    ...
    pg.draw.rect(
        display_surf, colors[color], (pixelx + 1, pixely + 1, block - 1, block - 1), 0, 3)
...
def gamecup(cup):
    ...
    pg.draw.rect(display_surf, brd_color, (side_margin - 4,
                 top_margin - 4, (cup_w * block) + 8, (cup_h * block) + 8), 5)

    # фон игрового поля
    pg.draw.rect(display_surf, bg_color, (side_margin,
                 top_margin, block * cup_w, block * cup_h))
    for x in range(cup_w):
